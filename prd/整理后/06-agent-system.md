...
Hook 有两个来源，由 Harness 加载合并后按事件驱动执行：

| 来源 | 位置 | 内容 | 说明 |
|------|------|------|------|
| **Agent Hooks** | `agents/{agent}/hooks/*.json` | Agent 自己声明的生命周期 hook | 每个 Agent 专属，跟随 Agent 代码版本走 |
| **Global Hooks** | `.actnow/settings.json` → `hooks` 字段 | 跨 Agent 的全局 hook（编排逻辑/通知/webhook） | 项目级配置，可 git 提交 |

**加载优先级**：`settings.json hooks > agent/hooks/`（同名 hook 以 settings.json 为准）

**合并逻辑**：Harness 启动时加载两处 hook，按 event 分组，agent 级 hook 先执行，settings.json 级 hook 后执行。
...
sequenceDiagram
    ...
    D->>H: G3完成
    H->>H: on_genesis_complete<br/>→ Canonical IR写入DB<br/>→ 前端刷新项目状态

    D->>H: 路由输出(含planned_actions)
    H->>H: on_approval_created<br/>→ 前端渲染可编辑气泡
    H->>H: 用户确认/拒绝
    H->>H: on_approval_resolved<br/>确认→写DB<br/>拒绝→通知Director重试

    Note over D,H: ═══ 制作阶段 ═══

    H->>SW: 触发编剧
    SW->>H: 剧本确认
    H->>H: on_script_draft<br/>→ 写入ScriptDraft<br/>→ 并行启动Asset+Designer

    par Asset
        AS->>H: Asset确认
        H->>H: on_asset_complete<br/>→ 写入Asset<br/>→ 通知Designer(asset_ready)
    and Designer
        Note over H: 等待Asset+Designer都确认
        GD->>H: Designer确认
        H->>H: on_design_complete<br/>→ 写入DesignPrompt<br/>→ check_gate: asset_and_design_ready<br/>→ 触发Storyboard
    end

    SB->>H: 分镜确认
    H->>H: on_storyboard_complete<br/>→ 写入Scene/Shot<br/>→ 推入生成任务
```

### Hook 详细规格

| # | Hook | 归属 Agent | 触发时机 | 动作 | 数据流 |
|---|------|-----------|----------|------|--------|
| H1 | `on_genesis_complete` | Director | G3 完成，canonical_ir 组装校验通过 | ① 写入 Project.canonicalIrJson ② emit project.canonical_ir.updated | canonical_ir → DB → 前端 |
| H2 | `on_approval_created` | Director | 路由输出含 needs_approval=true | ① emit ui.approval_card.show（含 planned_actions + editable_fields） | planned_actions → 前端可编辑气泡 |
| H3 | `on_approval_resolved` | Director | 用户点击确认或拒绝 | 确认：① 写入 DB ② emit agent.action.confirmed；拒绝：① emit agent.action.rejected ② 通知 Director 重新生成 | 用户操作 → DB / Director |
| H4 | `on_script_draft` | Screenwriter | 剧本确认 | ① 写入 ScriptDraft ② **并行**调用 Asset(asset_extraction) + Designer(design_prompt) | script_output → ScriptDraft + Asset + Designer |
| H5 | `on_script_revision` | Screenwriter | 剧本修改确认 | ① 写入 ScriptDraft ② 通知下游 Agent(affected: asset, designer, storyboard) | script_output → ScriptDraft + 下游通知 |
| H6 | `on_asset_complete` | Asset | Asset 确认 | ① 写入 Asset 表 ② signal designer: asset_ready | asset_output → DB + Designer |
| H7 | `on_design_complete` | Designer | Designer 确认 | ① 写入 DesignPrompt 表 ② check_gate: asset_and_design_ready → 触发 Storyboard | design_output → DB + Storyboard |
| H8 | `on_storyboard_complete` | Storyboard | 分镜确认 | ① 写入 Scene/Shot 表 ② create_generation_tasks(推入生成) | storyboard_output → DB + GenerationTask |

### Hook 间依赖关系

```mermaid
flowchart LR
    H1[on_genesis_complete] --> H2[on_approval_created]
    H2 --> H3[on_approval_resolved]
    H3 -->|确认| H4[on_script_draft]
    H3 -->|拒绝| H2
    H4 --> H6[on_asset_complete]
    H4 --> H7[on_design_complete]
    H6 -->|asset_ready| H7
    H7 -->|gate: both ready| H8[on_storyboard_complete]
    H5[on_script_revision] -->|通知| H6
    H5 -->|通知| H7
    H5 -->|通知| H8
```
...
1. **双来源加载**：Agent hooks（agent/hooks/）+ Global hooks（settings.json），Harness 合并后按 event 分组执行
2. **Agent hooks 跟代码走**：Agent 自己声明的生命周期 hook 存在 agent 目录，跟随 Agent 版本
3. **Global hooks 跟配置走**：编排逻辑（并行/通知/webhook）存在 settings.json，可按项目定制
4. **Hook 不做判断**：Hook 只执行确定性动作（写DB/emit事件/调用Agent），判断逻辑在 Agent 的 LLM 中
5. **并行执行**：on_script_draft 同时启动 Asset + Designer，两者并行完成后 check_gate 才触发 Storyboard
6. **拒绝回路**：on_approval_resolved 拒绝时，回路到对应 Agent 重新生成，不创建新的审批
7. **剧本修改通知**：on_script_revision 通知所有下游 Agent，由各 Agent 自行判断是否需要重新生成
8. **审批 hook 归属 Harness**：on_approval_created / on_approval_resolved 属于 Harness 层编排逻辑，应放在 settings.json 而非 agent 目录
