...
> **前端真源**：`../prototype/workspace.html`（单页四阶段，所有原型决策以此为准）
> ⚠️ **重要区分**（来源：tech/10-prototype-flow-alignment.md）：`apps/web`（React+React Flow）是后端 API 联调的测试工具页，**不是产品前端真源**。对齐原型产品流 = 让后端能力支撑 `workspace.html` 这条交互路径。
...
- 底部：**天眼层折叠条**（`👁 天眼层 · AI 暗骨推理 · 隐线/Canonical IR`，卡片内折叠，展开为占位内容）
...
**天眼层（暗骨/Canonical IR）折叠预留**：展开内容为占位，随灵感Agent提示词出参动态填充（当前为预留位）
...
flowchart TD
    ...
    Ask --> Conc[灵感模糊→具象]
    Conc --> Outline[大纲卡片出现在气泡内（可inline编辑）]
...
| 创意聊天台 | 对话流 + 气泡内大纲卡片/剧本卡片 + **右侧68px对话时间轴** | AI提问/给选项；大纲卡片inline编辑；确认大纲；进入制作 | 聊天室·可切全屏/侧边/悬浮球 | 时间轴进度层不随滚动变；阅读游标层scrollspy |
