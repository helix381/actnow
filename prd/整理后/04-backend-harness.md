...
| 编剧 Agent | 创意阶段专家 | 灵感发散、大纲（正面层+天眼层+剧本层三层出参）、剧本、情节/情绪节奏 | 不做分镜/资产 |
...
- **② 天眼层（Canonical IR）**：AI 暗骨推理，包含五大契约（mix/rules/arc/pressure/struct）+ assets + 线程账本 + 证据种子 + 推理日志；驱动后续制作的隐线，前端折叠预留位
